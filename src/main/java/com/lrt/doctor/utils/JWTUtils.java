...
public class JWTUtils {
    ...
    private static Integer expire = 24; // token 过期时间(小时)

    public static String getToken(String userId,String auth, String sign) {
        return JWT.create().withClaim("userId",userId) // 将userID保存到 token 里面,作为载荷
                .withClaim("role",auth) // 将用户的角色保存到token 中
